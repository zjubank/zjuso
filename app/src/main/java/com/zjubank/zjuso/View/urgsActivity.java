...
public class urgsActivity extends Activity implements GestureDetector.OnGestureListener {
...
//        tx.setText("本科生院办公网");
        ...
        BmobQuery<ugrs> overview_ugrs = new BmobQuery<>();
        overview_ugrs.setLimit(20);
        overview_ugrs.order("-date");
        overview_ugrs.findObjects(this, new FindListener<ugrs>() {
            @Override
            public void onSuccess(List<ugrs> list) {
                for( ugrs l : list )
                {
...
//        }
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (e1.getX() - e2.getX() > 120) {
            Toast.makeText(getApplicationContext(), "已经到最右边啦（≧∇≦）", Toast.LENGTH_SHORT).show();
            return true;
        } else if (e1.getX() - e2.getX() < -120) {
            Intent intent = new Intent();
            intent.setClass(urgsActivity.this, cspoActivity.class);
            urgsActivity.this.startActivity(intent);
