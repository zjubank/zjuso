...
public class cspoActivity extends Activity implements GestureDetector.OnGestureListener {
...
//        tx.setText("计算机学院办公网");
        ...
        BmobQuery<cspo> overview_cspo = new BmobQuery<>();
        overview_cspo.setLimit(20);
        overview_cspo.order("-date");
        overview_cspo.findObjects(this, new FindListener<cspo>() {
            @Override
            public void onSuccess(List<cspo> list) {
                for( cspo l : list )
                {
...
//        }
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (e1.getX() - e2.getX() > 120) {
            ...
            intent.setClass(cspoActivity.this, urgsActivity.class);
            cspoActivity.this.startActivity(intent);
            overridePendingTransition(R.anim.push_left_in,
                    R.anim.push_left_out);
            return true;
        } else if (e1.getX() - e2.getX() < -120) {
            Intent intent = new Intent();
            intent.setClass(cspoActivity.this, ckcActivity.class);
            cspoActivity.this.startActivity(intent);
